 ...
 * HISTORY
 * $Log:	routine.c,v $
 * Revision 2.11  93/11/17  18:48:28  dbg
 ...
 * 
 * Revision 2.10  93/05/10  17:49:31  rvb
 * 	Fix include to use < vs " for new ode shadowing
 * 	[93/05/10  10:28:28  rvb]
 * 
 * Revision 2.9  93/01/14  17:58:30  danner
...
rtAddCountArg(register argument_t *arg)
{
	...
	suff = "msgt_number";
	count->argSuffix = suff;
    }
    count->argMsgField = strconcat3(arg->argTTName, ".", suff);
}
...
rtAddPolyArg(register argument_t *arg)
{
    ...
    poly->argMsgField = strconcat3(arg->argTTName, ".", suff);
}
...
rtAddDeallocArg(register argument_t *arg)
{
    ...
    if (arg->argLongForm)
	suff = "msgtl_header.msgt_deallocate";
    else {
	suff = "msgt_deallocate";
	dealloc->argSuffix = suff;
    }
    dealloc->argMsgField = strconcat3(arg->argTTName, ".", suff);
}
...
rtAddSCopyArg(register argument_t *arg)
{
    ...
    if (arg->argLongForm)
	suff = "msgtl_header.msgt_inline";
    else {
	suff = "msgt_inline";
	scopy->argSuffix = suff;
    }
    scopy->argMsgField = strconcat3(arg->argTTName, ".", suff);
}
...
rtCheckRoutineArgs(routine_t *rt)
{
	...
	if (it != itNULL)
	{
		...
		(it->itOutName == MACH_MSG_TYPE_POLYMORPHIC))
		rtAddPolyArg(arg);
	    /* backtrack possible mistake */
	    if ((rt->rtRequestPort && arg == rt->rtRequestPort->argPoly) ||
		(rt->rtReplyPort && arg == rt->rtReplyPort->argPoly))
		arg->argSuffix = strNULL;
	}
    }
}
