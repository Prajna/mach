 ...
 * HISTORY
 * $Log:	ldx.c,v $
 * Revision 2.1.1.1  94/06/01  10:19:44  rvb
 * 	From BSDSS
 * 
 * Revision 2.2  93/05/11  11:42:11  rvb
 * 		Gnu code originally used by Mach I386 release -> BSDSS
 * 	[93/05/05  15:21:25  rvb]
 * 
 *
 */
/*
...
digest_symbols ()
{
  ...
  if (! Tdata_flag_specified)
    ...
    if (magic == NMAGIC || magic == ZMAGIC)
	data_start = (data_start + SEGMENT_SIZE - 1) & (- SEGMENT_SIZE);
#endif	NOT_MACH
     ...
     FILE *outfile;
{
  ...
  if (list_warning_symbols)
    each_file (mark_flagged_symbols, outfile);
...
write_header ()
{
  N_SET_MAGIC (outheader, magic);
#ifdef	MACH_HOLE
  if ((T_flag_specified || Tdata_flag_specified) && !(text_start&0xf0000000)) /* this last conjunct is a tmp hack */
    {
