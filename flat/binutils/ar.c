 ...
 * HISTORY
 * $Log:	ar.c,v $
 * Revision 2.1.1.1  94/06/01  10:19:38  rvb
 * 	From BSDSS
 * 
 * Revision 2.3  93/06/02  12:02:43  rvb
 * 	cast to quiet gcc
 * 
 * Revision 2.2  93/05/11  11:42:01  rvb
 * 		Gnu code originally used by Mach I386 release -> BSDSS
 * 	[93/05/05  15:21:18  rvb]
 * 
 *
 */
/*
     ...
     struct member_desc member;
{
  ...
  timestring = ctime ((const time_t *)&member.date); /* int -> time_t (long) */
...
#ifdef	NOT_MACH
#else	NOT_MACH
	  if (!(int) tail->info.name)	/* insert_in_map zero's name to indicate */
	  	continue;		/* replaced entry */
#endif	NOT_MACH
